...
from ariadne_wavelet_attnbias import parameter
from ariadne_wavelet_attnbias.evaluation import save_evaluation_metrics_plot
...
from ariadne_wavelet_attnbias.scripts.train_attnbias import build_runtime_config, parse_args
from ariadne_wavelet_attnbias.training_monitor import TrainingMonitor
...
class ArtifactLayoutTests(unittest.TestCase):
    ...
    def test_train_parse_args_rejects_resume_conflicts(self):
        with patch.object(sys, "argv", ["train_attnbias.py", "--smoke", "--resume-from", "/tmp/checkpoint.pth"]), patch(
            "ariadne_wavelet_attnbias.scripts.train_attnbias.resolve_resume_checkpoint",
            return_value=(Path("/tmp/checkpoint.pth"), "2026_0303_0003"),
        ), patch(
            "ariadne_wavelet_attnbias.scripts.train_attnbias.get_run_identity_from_checkpoint",
    ...
    def test_train_parse_args_accepts_resume(self):
        with patch.object(sys, "argv", ["train_attnbias.py", "--resume-from", "/tmp/checkpoint.pth"]), patch(
            "ariadne_wavelet_attnbias.scripts.train_attnbias.resolve_resume_checkpoint",
            return_value=(Path("/tmp/checkpoint.pth"), "2026_0303_0003"),
        ), patch(
            "ariadne_wavelet_attnbias.scripts.train_attnbias.get_run_identity_from_checkpoint",
    ...
    def test_smoke_config_keeps_default_gpu_settings(self):
        ...
        with patch.object(sys, "argv", ["train_attnbias.py", "--smoke"]):
            args = parse_args()
    ...
    def test_train_parse_args_applies_gif_frame_rate(self):
        with patch.object(sys, "argv", ["train_attnbias.py", "--gif-frame-rate", "0.8"]):
            args = parse_args()
        config = build_runtime_config(args)
