...
class NodeManager:
    ...
    def update_graph(self, robot_location, frontiers, updating_map_info, map_info):
        ...
        wavelet_map = multiscale_haar_energy_map(updating_map_info.map, scales=parameter.WAVELET_SCALES)
        ...
        for coords in node_coords:
            ...
            node.wavelet = wavelet_score_at_coords(node.coords, updating_map_info, wavelet_map)
    ...
    def adaptive_distance_threshold(self, ref_coords, map_info, wavelet_map):
        ...
        wavelet = wavelet_score_at_coords(np.array(ref_coords), map_info, wavelet_map)
        mult = min(
    ...
    def _get_or_create_key_node(self, coords, map_info, wavelet_map, node=None):
        ...
        key_node = KeyNode(
            ...
            node.utility,
            node.visited,
            wavelet_score_at_coords(node.coords, map_info, wavelet_map),
        )
    ...
    def get_rarefied_graph(self, robot_location, map_info):
        self.dist_to_nearest_frontier = 1e8
        wavelet_map = multiscale_haar_energy_map(map_info.map, scales=parameter.WAVELET_SCALES)
...
class KeyNode:
    def __init__(self, coords, utility, visited, wavelet):
        self.coords = coords
