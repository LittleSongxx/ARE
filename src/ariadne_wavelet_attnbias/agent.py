...
class Agent(BaseAgent):
    def __init__(self, policy_net, device="cpu", plot=False):
        super().__init__(policy_net, device=device, plot=plot)
        self.node_manager = TrainNodeManager(plot=self.plot)
        self.wavelet = None
    ...
    def update_observation(self):
        ...
        return all_node_coords, utility, guidepost, wavelet, adjacent_matrix, current_index, neighbor_indices

    def get_observation(self):
        node_coords = self.node_coords
        node_utility = self.utility.reshape(-1, 1)
        node_guidepost = self.guidepost.reshape(-1, 1)
        node_wavelet = self.wavelet.reshape(-1, 1)
        ...
        node_inputs = np.concatenate((node_coords, node_utility, node_guidepost, node_wavelet), axis=1)
        assert node_inputs.shape[1] == NODE_INPUT_DIM
...
class InferenceAgent(BaseAgent):
    ...
    def update_dense_observation(self):
        ...
        for i, coords in enumerate(all_node_coords):
            ...
            utility.append(node.utility)
            guidepost.append(node.visited)
            wavelet.append(getattr(node, "wavelet", 0.0))
            for neighbor in node.neighbor_set:
                index = np.argwhere(node_coords_to_check == neighbor[0] + neighbor[1] * 1j)
                index = index[0][0]
                adjacent_matrix[i, index] = 0
    ...
    def get_observation(self, robot_location):
        ...
        node_wavelet = self.key_wavelet.reshape(-1, 1)
