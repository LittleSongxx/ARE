...
from .utils import make_attn_bias_from_node_inputs
...
class MultiHeadAttention(nn.Module):
    ...
    def _prepare_attn_bias(self, attn_bias, n_batch, n_query, n_key, reference):
        if attn_bias.dim() == 3:
            ...
            if attn_bias.size(0) == 1 and tuple(attn_bias.shape[1:]) == (n_batch, n_query, n_key):
                bias = attn_bias
            elif attn_bias.size(1) == 1 and tuple(attn_bias.shape) == (n_batch, 1, n_query, n_key):
    ...
    def forward(self, q, k=None, v=None, key_padding_mask=None, attn_mask=None, attn_bias=None):
        ...
        if attn_bias is not None and parameter.USE_ATTENTION_BIAS and parameter.ATTN_BIAS_APPLY_ENCODER:
            U = U + parameter.ATTN_BIAS_BETA * self._prepare_attn_bias(attn_bias, n_batch, n_query, n_key, U)
...
class PolicyNet(nn.Module):
    ...
    def encode_graph(self, node_inputs, node_padding_mask, edge_mask):
        attn_bias = None
        if parameter.USE_ATTENTION_BIAS and parameter.ATTN_BIAS_APPLY_ENCODER:
            attn_bias = make_attn_bias_from_node_inputs(node_inputs, node_padding_mask=node_padding_mask)
