...
class MultiHeadAttention(nn.Module):
    ...
    def _prepare_attn_bias(self, attn_bias, n_batch, n_query, n_key, reference):
        if attn_bias.dim() == 3:
            if tuple(attn_bias.shape) != (n_batch, n_query, n_key):
                raise ValueError(
                    f"attn_bias shape {tuple(attn_bias.shape)} does not match expected {(n_batch, n_query, n_key)}"
                )
            bias = attn_bias.unsqueeze(0)
        elif attn_bias.dim() == 4:
            if tuple(attn_bias.shape) == (n_batch, 1, n_query, n_key):
                bias = attn_bias.permute(1, 0, 2, 3)
            elif tuple(attn_bias.shape) == (1, n_batch, n_query, n_key):
                bias = attn_bias
            else:
                raise ValueError(
                    "attn_bias must have shape [B, Nq, Nk], [B, 1, Nq, Nk], or [1, B, Nq, Nk], "
        ...
        return bias.to(device=reference.device, dtype=reference.dtype).expand(self.n_heads, n_batch, n_query, n_key)
    ...
    def forward(self, q, k=None, v=None, key_padding_mask=None, attn_mask=None, attn_bias=None):
        ...
        if attn_bias is not None:
            logits = logits + self._prepare_attn_bias(attn_bias, n_batch, n_query, n_key, logits)
...
class EncoderLayer(nn.Module):
    ...
    def forward(self, src, key_padding_mask=None, attn_mask=None, attn_bias=None):
        h0 = src
        h = self.normalization1(src)
        h, _ = self.multiHeadAttention(
            q=h,
...
class Encoder(nn.Module):
    ...
    def forward(self, src, key_padding_mask=None, attn_mask=None, attn_bias=None):
        for layer in self.layers:
            src = layer(src, key_padding_mask=key_padding_mask, attn_mask=attn_mask, attn_bias=attn_bias)
        return src
...
class PolicyNet(nn.Module):
    ...
    def encode_graph(self, node_inputs, node_padding_mask, edge_mask, attn_bias=None):
    ...
    def forward(self, node_inputs, node_padding_mask, edge_mask, current_index, current_edge, edge_padding_mask, attn_bias=None):
        enhanced_node_feature = self.encode_graph(node_inputs, node_padding_mask, edge_mask, attn_bias=attn_bias)
...
class QNet(nn.Module):
    ...
    def output_q(self, current_node_feature, enhanced_current_node_feature, enhanced_node_feature, current_edge, edge_padding_mask):
        embedding_dim = enhanced_node_feature.size()[2]
