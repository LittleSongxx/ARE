...
class NodeManager:
    def __init__(self, runtime_config=None, start=np.array([0,0])):
        self.runtime_config = runtime_config or parameter.RuntimeConfig()
    ...
    def _needs_wavelet_maps(self):
        return (
            self.runtime_config.use_wavelet_feature
            or self.runtime_config.use_wavelet_attn_bias
            or self.runtime_config.wavelet_skip_utility_updates
            or self.runtime_config.wavelet_guided_node_sampling
            or self.runtime_config.wavelet_adaptive_dth
        )

    def _update_node_wavelet(self, node, map_info, wavelet_maps):
        if wavelet_maps is None:
        ...
        node.wavelet_scalar = wavelet_scalar_at_coords(node.coords, map_info, wavelet_maps)
        node.wavelet = float(node.wavelet_scalar)

    def _guided_sample_node_coords(self, node_coords, robot_location, map_info, wavelet_maps):
        if (
            not self.runtime_config.wavelet_guided_node_sampling
            or wavelet_maps is None
            or len(node_coords) <= self.runtime_config.wavelet_node_min_keep
        ):
            return node_coords
        ...
        keep = scores >= self.runtime_config.wavelet_node_keep_thresh
        keep |= np.random.rand(len(node_coords)) < self.runtime_config.wavelet_node_keep_prob_low

        if self.runtime_config.wavelet_node_always_keep_current_and_neighbors:
            distances = np.linalg.norm(node_coords - robot_location.reshape(1, 2), axis=1)
            keep |= distances <= np.sqrt(2.0) * parameter.NODE_RESOLUTION + 1e-6

        if int(np.sum(keep)) < self.runtime_config.wavelet_node_min_keep:
    ...
    def _should_skip_utility_update(self, node, robot_location):
        if not self.runtime_config.wavelet_skip_utility_updates:
            return False
        if node.wavelet_scalar >= self.runtime_config.wavelet_skip_thresh:
            return False
        if node.utility >= self.runtime_config.wavelet_skip_utility_low:
            return False
        if np.linalg.norm(node.coords - robot_location) <= self.runtime_config.wavelet_skip_near_robot_radius:
            return False
        if node.steps_since_last_update >= self.runtime_config.wavelet_skip_max_age_steps:
            return False
        return True
    ...
    def update_graph(self, robot_location, frontiers, updating_map_info, map_info):
        ...
        wavelet_maps = None
        if self._needs_wavelet_maps():
            wavelet_maps = compute_wavelet_maps(updating_map_info.map, self.runtime_config)
            node_coords = self._guided_sample_node_coords(node_coords, robot_location, updating_map_info, wavelet_maps)
        ...
        for coords in node_coords:
            ...
            if node is None:
                ...
                self._update_node_wavelet(node, updating_map_info, wavelet_maps)
            else:
                node = node.data
                self._update_node_wavelet(node, updating_map_info, wavelet_maps)
        ...
        if current_node is not None:
            ...
            LOGGER.info("The current node should be removed.")
    ...
    def adaptive_distance_threshold(self, ref_coords, map_info, wavelet_map):
        base = parameter.THR_NEXT_WAYPOINT + parameter.NODE_RESOLUTION
        if not self.runtime_config.wavelet_adaptive_dth or wavelet_map is None:
            return base
        wavelet = wavelet_scalar_at_coords(np.array(ref_coords), map_info, wavelet_map)
        mult = min(
            1.0 + self.runtime_config.wavelet_dth_alpha * (1.0 - wavelet),
            self.runtime_config.wavelet_dth_max_mult,
        )
        return base * mult

    def _get_or_create_key_node(self, coords, map_info, wavelet_map, node=None):
        key = (coords[0], coords[1])
        if key in self.key_node_dict:
            return self.key_node_dict[key]
        if node is None:
            node = self.nodes_dict.find(key).data
        key_node = KeyNode(
            node.coords,
            node.utility,
            node.visited,
            getattr(node, "wavelet_feat", np.zeros((0,), dtype=np.float32)),
            getattr(node, "wavelet_scalar", 0.0),
        )
        self.key_node_dict[key] = key_node
        return key_node

    def get_rarefied_graph(self, robot_location, map_info):
        self.dist_to_nearest_frontier = 1e8
        wavelet_map = compute_wavelet_maps(map_info.map, self.runtime_config) if self._needs_wavelet_maps() else None
        ...
        key_current_node = self._get_or_create_key_node(current_node.coords, map_info, wavelet_map, node=current_node)
                    ...
                    if first_node:
                        ...
                        if coords not in self.nodes_dict.find(ref).data.neighbor_set and (
                            ...
                            np.array(coords) - np.array(ref)) > (
                                        self.adaptive_distance_threshold(ref, map_info, wavelet_map))):
                            assert i >= 1, print(self.nodes_dict.find(ref).data.neighbor_set, path, dist)

                            key_node_coords = path[i - 1]
                            key_node = self._get_or_create_key_node(key_node_coords, map_info, wavelet_map)
                            key_node.add_neighbor_coords(ref)
                            ref_node.add_neighbor_coords(key_node_coords)
                            first_node = False

                            key_node_coords = coords
                            self._get_or_create_key_node(key_node_coords, map_info, wavelet_map)
        ...
        for center_node, dstar in self.cluster_center_node_dict.values():
            ...
            if dstar is not None:
                ...
                if dist == 1e8:
                    LOGGER.debug("D*Lite exceeded time limit")
            else:
            ...
            for i, coords in enumerate(path):
                if first_node:
                    if coords not in self.nodes_dict.find(ref).data.neighbor_set and (
                            check_collision(np.array(ref), np.array(coords), map_info) or np.linalg.norm(
                        np.array(coords) - np.array(ref)) > self.adaptive_distance_threshold(ref, map_info, wavelet_map)):
                        assert i >= 1, print(self.nodes_dict.find(ref).data.neighbor_set, path, dist)
                        key_node_coords = path[i - 1]
                        key_node = self._get_or_create_key_node(key_node_coords, map_info, wavelet_map)
                        key_node.add_neighbor_coords(ref)
        ...
        for node in non_zero_utility_nodes:
            ...
            if coords not in self.key_node_dict.keys():
                self._get_or_create_key_node(node.coords, map_info, wavelet_map, node=node)
...
class KeyNode:
    def __init__(self, coords, utility, visited, wavelet_feat, wavelet_scalar):
        self.coords = coords
        self.utility = utility
        self.visited = visited
        self.wavelet_feat = np.asarray(wavelet_feat, dtype=np.float32)
        self.wavelet_scalar = float(wavelet_scalar)
        self.wavelet = self.wavelet_scalar
        self.neighbor_set = set()
        self.neighbor_set.add((self.coords[0], self.coords[1]))
