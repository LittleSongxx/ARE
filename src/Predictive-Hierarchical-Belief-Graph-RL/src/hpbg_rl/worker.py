...
from .env import Env
from .expert_reward import apply_expert_reward, compute_state_exploration_potential
...
class Worker:
    def __init__(
        ...
        self.gif_episode_dir = (
            ensure_episode_bucket_dir(
                get_gifs_path(self.runtime_config),
                global_step,
                self.runtime_config.result_bucket_episodes,
            )
            if self.save_image
            else None
        )
        ...
        self.env = Env(
            ...
            gifs_dir=self.gif_episode_dir,
            runtime_config=self.runtime_config,
        )
        self.robot = Agent(policy_net, self.device, self.save_image, runtime_config=self.runtime_config)
        ...
        self.shaped_episode_return = 0.0

    def _state_potential(self) -> float:
        if not (self.runtime_config.use_hpbg and self.runtime_config.use_expert_reward):
            return 0.0
        return compute_state_exploration_potential(self.env.robot_belief, self.env.ground_truth)

    def _shape_reward(self, raw_reward: float, previous_potential: float, next_potential: float) -> float:
        if not (
            self.runtime_config.use_hpbg
            and self.runtime_config.use_expert_reward
            and self.runtime_config.hpbg_expert_reward_weight > 0
        ):
            return float(raw_reward)
        return apply_expert_reward(
            raw_reward,
            previous_potential,
            next_potential,
            GAMMA,
            self.runtime_config.hpbg_expert_reward_weight * self.runtime_config.hpbg_expert_potential_weight,
            self.runtime_config.hpbg_oracle_gain_weight,
        )
    ...
    def run_episode(self):
        ...
        for i in range(self.runtime_config.max_episode_step):
            ...
            previous_potential = self._state_potential()
            raw_reward = self.env.step(next_location)
            reward = raw_reward
            ...
            if self.robot.utility.sum() == 0:
                ...
                reward += 20
                raw_reward += 20
            next_potential = 0.0 if done else self._state_potential()
            reward = self._shape_reward(reward, previous_potential, next_potential)
            self.raw_episode_return += float(raw_reward)
            self.shaped_episode_return += float(reward)
        ...
        self.perf_metrics["episode_return"] = self.shaped_episode_return
        self.perf_metrics["episode_raw_return"] = self.raw_episode_return
        self.perf_metrics["expert_reward_delta"] = self.shaped_episode_return - self.raw_episode_return
