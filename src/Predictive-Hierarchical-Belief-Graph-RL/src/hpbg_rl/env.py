...
from .corridor_refinement import compute_smoothness_penalty
from .map_splits import resolve_map_split_paths
...
class Env:
    def __init__(
        ...
        self.enable_smoothness_reward = self.runtime_config.enable_smoothness_reward
        self.smoothness_turn_penalty = self.runtime_config.smoothness_turn_penalty
        self.smoothness_lateral_penalty = self.runtime_config.smoothness_lateral_penalty
        self.previous_robot_location = None
    ...
    def import_ground_truth(self, episode_index):
        if self.forced_map_path is not None:
            ...
            map_list = resolve_map_split_paths(self.runtime_config, "train", allow_train_eval=True)
            if not map_list:
                raise FileNotFoundError("No train maps available in the configured split manifest")
    ...
    def calculate_reward(self, dist):
        ...
        reward += delta_num / (SENSOR_RANGE * 3.14 // (2 * CELL_SIZE))
    ...
    def step(self, next_waypoint):
        current_location = np.asarray(self.robot_location, dtype=float).copy()
        smoothness_penalty = 0.0
        if self.enable_smoothness_reward:
            smoothness_penalty = compute_smoothness_penalty(
                self.previous_robot_location,
                current_location,
                next_waypoint,
                turn_penalty_weight=self.smoothness_turn_penalty,
                lateral_penalty_weight=self.smoothness_lateral_penalty,
            )

        dist = np.linalg.norm(current_location - next_waypoint)
        ...
        self.previous_robot_location = current_location
        return reward
