...
from ariadne_wavelet.parameter import RuntimeConfig, NUM_META_AGENT
from ariadne_wavelet.runtime_utils import (
...
class RayWorkerConfigTests(unittest.TestCase):
    ...
    def test_auto_detect_worker_threads_keeps_single_core_hosts_single_threaded(self):
        with mock.patch("os.cpu_count", return_value=1):
            self.assertEqual(_auto_detect_worker_threads(NUM_META_AGENT), 1)
