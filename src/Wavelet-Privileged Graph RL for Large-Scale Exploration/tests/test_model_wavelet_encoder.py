...
from wpg_rl.model import PolicyNet, QNet
from wpg_rl.parameter import EMBEDDING_DIM, K_SIZE, NODE_INPUT_DIM
...
class ModelWaveletEncoderTests(unittest.TestCase):
    def test_baseline_shapes_with_return_hidden(self):
        ...
        critic = QNet(NODE_INPUT_DIM + 1, EMBEDDING_DIM)

        logp, hidden = policy(*obs, return_hidden=True)
        critic_inputs = obs.copy()
        critic_inputs[0] = torch.randn(obs[0].size(0), obs[0].size(1), NODE_INPUT_DIM + 1)
