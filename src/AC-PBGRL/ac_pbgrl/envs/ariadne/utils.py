...
from .parameter import CELL_SIZE, FRONTIER_CELL_SIZE, FREE, NODE_RESOLUTION, OCCUPIED, RESULT_BUCKET_EPISODES, UNKNOWN
...
def make_gif(path, n, frame_files, rate):
    gif_path = Path(path) / f"{n}_explored_rate_{rate:.4g}.gif"
    with imageio.get_writer(str(gif_path), mode="I", duration=0.5) as writer:
...
def get_episode_bucket_name(episode, bucket_size=RESULT_BUCKET_EPISODES):
    episode = max(int(episode), 1)
    bucket_size = max(int(bucket_size), 1)
    end = ((episode - 1) // bucket_size + 1) * bucket_size
    return f"episode_{end}"
...
def ensure_episode_bucket_dir(base_dir, episode, bucket_size=RESULT_BUCKET_EPISODES):
    bucket_dir = Path(base_dir) / get_episode_bucket_name(episode, bucket_size)
    bucket_dir.mkdir(parents=True, exist_ok=True)
    return bucket_dir
