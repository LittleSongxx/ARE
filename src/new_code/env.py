...
class Env:
    ...
    def import_ground_truth(self, episode_index):
        ...
        map_list = sorted(path for path in MAPS_DIR.iterdir() if path.is_file())
        if not map_list:
            raise FileNotFoundError(f"No maps found in MAPS_DIR: {MAPS_DIR}")
        map_index = episode_index % np.size(map_list)
        ground_truth = (io.imread(str(map_list[map_index]), 1) * 255).astype(int)
    ...
    def plot_env(self, step, output_dir=None):
        ...
        plt.tight_layout()
        plt.savefig('{}/step_{}.png'.format(episode_gifs_path, step), dpi=150)
        frame = '{}/step_{}.png'.format(episode_gifs_path, step)
        plt.close()
