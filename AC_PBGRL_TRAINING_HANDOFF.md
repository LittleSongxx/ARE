...
最后更新：2026-08-27 21:15（Asia/Shanghai）
当前分支：`dev_kf`  
核心实现基线：`53f80932`
最新流水线计划测试：`c3c64d75`
...
- train manifest 已达到 15,360 samples / 15 shards；75% 聚合审计确认 manifest、分片样本数和路径一一对应，全部 249,861 个有效候选标签、node/edge features 与 mask 均通过完整性检查，teacher/map-split provenance 哈希一致。
- 75% 标签分布未退化：均值 `-0.829`、标准差 `0.758`、范围 `[-3.746, 2.610]`，按六位小数约有 182,955 个唯一值。
