...
class Agent:
    def __init__(self, policy_net, device="cpu", plot=False):
        ...
        self.enable_corridor_graph_compression = ENABLE_CORRIDOR_GRAPH_COMPRESSION
        self.enable_corridor_edge_pruning = ENABLE_CORRIDOR_EDGE_PRUNING
    ...
    def get_next_observation(self, next_node_index, observation):
        node_inputs, node_padding_mask, edge_mask, current_index_tensor, _, _ = observation
        next_edge = torch.argwhere(edge_mask[0, next_node_index] == 0).flatten()

        next_in_edge_matches = torch.argwhere(next_edge == next_node_index)
        current_idx_val = current_index_tensor.item()
        current_in_edge_matches = torch.argwhere(next_edge == current_idx_val)

        k_size = next_edge.size()[-1]
        if k_size > K_SIZE:
            next_edge = next_edge[:K_SIZE]
            k_size = K_SIZE
            next_in_edge_matches = torch.argwhere(next_edge == next_node_index)
            current_in_edge_matches = torch.argwhere(next_edge == current_idx_val)

        padding_pad = torch.nn.ConstantPad1d((0, K_SIZE - k_size), 0)
        next_edge = padding_pad(next_edge.unsqueeze(0)).unsqueeze(-1).to(self.device)

        next_node_index_tensor = torch.tensor([next_node_index]).reshape(1, 1, 1).to(self.device)
        edge_padding_mask = torch.zeros((1, 1, k_size), dtype=torch.int16).to(self.device)
        if next_in_edge_matches.numel() > 0:
            edge_padding_mask[0, 0, next_in_edge_matches[0].item()] = 1
        if current_in_edge_matches.numel() > 0:
            edge_padding_mask[0, 0, current_in_edge_matches[0].item()] = 1
        mask_padding = torch.nn.ConstantPad1d((0, K_SIZE - k_size), 1)
        edge_padding_mask = mask_padding(edge_padding_mask)

        return [node_inputs, node_padding_mask, edge_mask, next_node_index_tensor, next_edge, edge_padding_mask]
