...
from .utils import (
    ...
    multiscale_haar_energy_map,
    wavelet_score_at_coords,
)
...
class GroundTruthNodeManager:
    def __init__(self, node_manager, ground_truth_map_info, device="cpu", plot=False):
        ...
        self.device = device
        self.plot = plot
        self.wavelet_map = multiscale_haar_energy_map(
            self.ground_truth_map_info.map,
    ...
    def get_ground_truth_observation(self, robot_location):
        ...
        wavelet = np.array(wavelet).reshape(-1, 1)
        ...
        node_inputs = np.concatenate(
            (node_coords, node_utility, node_explored, node_guidepost, wavelet),
            axis=1,
        )
        ...
        edge_mask = torch.tensor(edge_mask, device=self.device).unsqueeze(0)
    ...
    def initialize_graph(self):
        ...
        for coords in node_coords:
            ...
            node.wavelet = wavelet_score_at_coords(coords, self.ground_truth_map_info, self.wavelet_map)
    ...
    def update_graph(self):
        for node in self.node_manager.nodes_dict.__iter__():
            ...
            ground_truth_node.data.wavelet = wavelet_score_at_coords(
                coords,
